...
const EmailFilter = () => {
  ...
  return (
    <>
      <div className="left-part max-w-60 h-full w-full ">
        <EmailCompose />
        <ul className="my-4">
          {filterData.map((item) => {
            ...
            return (
              <li
                key={item.id}
                className={`py-2.5 gap-2 px-4 rounded-md cursor-pointer capitalize flex hover:bg-lighthover dark:hover:bg-darkmuted mb-0.5 ${filter === item.name ? "text-primary bg-lighthover dark:bg-darkmuted" : "bg-hover  rounded-md text-ld"}`}

...
interface ProductContextType {
  ...
  addToCart: (productId: number | string, qty?: number) => void
...
export const ProductProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const addToCart = async (productId: number | string, qty: number = 1) => {
    try {
      await cartMutate(postFetcher('/api/eCommerce/carts', { productId, qty }))
    } catch (error) {
